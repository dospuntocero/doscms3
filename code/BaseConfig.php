...
class BaseConfig extends DataExtension {
  ...
  function updateCMSFields(FieldList $fields) {
    ...
    $addThisID->setRightTitle(_t('BaseConfig.ADDTHISPROFILE',"Profile ID to be used all across the site (in the format <strong>ra-XXXXXXXXXXXXXXXX</strong>)"));

    $fields->addFieldToTab('Root.Logos', $logoField = new UploadField('Logo', _t('BaseConfig.SITELOGO',"Logo, to appear in the top left.")));
    $logoField->getValidator()->setAllowedExtensions(array('jpg', 'jpeg', 'png', 'gif'));
    $logoField->setConfig('allowedMaxFileNumber', 1);

    $fields->addFieldToTab('Root.Logos', $footerLogoField = new UploadField('FooterLogo', _t('BaseConfig.FOOTERLOGO',"Footer logo, to appear in the bottom right.")));
    $footerLogoField->getValidator()->setAllowedExtensions(array('jpg', 'jpeg', 'png', 'gif'));
    $footerLogoField->setConfig('allowedMaxFileNumber', 1);

    $fields->addFieldToTab('Root.Logos', $footerLink = new TextField('FooterLogoLink', _t('BaseConfig.FOOTERLOGOLINK',"Footer Logo link")));
    $footerLink->setRightTitle(_t('BaseConfig.FOOTERLOGOLINKINSTRUCTIONS',"Please include the protocol (ie, http:// or https://) unless it is an internal link."));

    $fields->addFieldToTab('Root.Logos', new TextField('FooterLogoDescription', _t('BaseConfig.FOOTERLOGODESCRIPTION',"Footer Logo description")));
  }
}